...
namespace Server.Engines.Craft
{
    ...
    public class CraftItem
    {
        ...
        private static readonly Type[][] m_TypesTable =
        {
            ...
            new[] { typeof(FrostbarkLog),  typeof(FrostbarkBoard)  },
            new[] { typeof(ShadowbarkLog), typeof(ShadowbarkBoard) },
            new[] { typeof(RunewoodLog),   typeof(RunewoodBoard)   },
            new[] { typeof(VoidwoodLog),   typeof(VoidwoodBoard)   },
            new[] { typeof(StarwoodLog),   typeof(StarwoodBoard)   },
        ...
        )
        {
            ...
            // Dev bypass -- skip all resource and proximity requirements for testing
            if (from is PlayerMobile devBypassPm2 && DevTestingCrystal.IsActive(devBypassPm2))
                return true;
            ...
            for (var i = 0; i < types.Length; ++i)
            {
                ...
                if (UseAllRes)
                {
                    var tempAmount = ourPack.GetBestGroupAmount(types[i], true, CheckHueGrouping);
        ...
        {
            // Dev bypass -- full skill and success-chance bypass for testing
            if (from is PlayerMobile devBypassPm && DevTestingCrystal.IsActive(devBypassPm))
            {
        ...
        )
        {
            ...
            if (CheckSkills(from, typeRes, craftSystem, ref ignored, out var allRequiredSkills))
            {
                ...
                if (craftSystem is DefBlacksmithy)
                {
                    ...
                    // ClusterF: Metal Familiarity tracking for Hammer of Hephaestus
                    if (tool is HammerOfHephaestus clusterHoh)
                        clusterHoh.RecordFamiliarity(typeRes, from);
                    else if (tool is ReinforcedHammerOfHephaestus clusterRhoh)
                        clusterRhoh.RecordFamiliarity(typeRes, from);
                }
                ...
                if (item != null)
                {
                    ...
                    // ClusterF: Crafting achievements
                    if (from is PlayerMobile clusterAchPm)
                    {
                        var clusterIsExc = item switch
                        {
                            BaseWeapon  bw  => bw.Quality  == WeaponQuality.Exceptional,
                            BaseArmor   ba  => ba.Quality  == ArmorQuality.Exceptional,
                            BaseClothing cl => cl.Quality  == ClothingQuality.Exceptional,
                            _               => false,
                        };
                        if (craftSystem is DefBlacksmithy)
                            ClusterFAchievementSystem.NotifySmithedItem(clusterAchPm, clusterIsExc);
                        else if (craftSystem is DefTailoring)
                            ClusterFAchievementSystem.NotifyTailoredItem(clusterAchPm, clusterIsExc);
                        else
                            ClusterFAchievementSystem.NotifyCraftedItem(clusterAchPm, clusterIsExc);
                    }
