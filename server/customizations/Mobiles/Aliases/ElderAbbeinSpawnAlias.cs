...
// 0d1fffd9f) asks for "abbein" at Heartwood [7048,382] on Trammel and Felucca (shared/*/TownsPeople.json), and no
// assembly declares that name, so BaseSpawner runs the entry with EntryFlags.InvalidType (BaseSpawner.cs:1130) and the
// spot stays empty. The type the data means is stock ElderAbbein (Engines/ML Quests/Definitions/Heartwood.cs:3361). Its
// DefaultName is "Elder Abbein", title "the wise", a female elf with AI_Vendor; ServUO's Mobiles/NPCs/Abbein.cs is the
// same NPC by name, title and sex (a BaseVendor that sells nothing and offers no quest). Pinned's data names
// elderabbein nowhere, so under stock ModernUO Heartwood has no Elder Abbein.
//
...
// "Server.Engines.MLQuests.Definitions.ElderAbbein", and nothing a player sees changes.
//
...
[TypeAlias("Server.Engines.MLQuests.Definitions.Abbein")]
public partial class ElderAbbein
{
}
