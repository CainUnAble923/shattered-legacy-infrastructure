...
// 0d1fffd9f) asks for "vicaie" at Heartwood [7048,382] on Trammel and Felucca (shared/*/TownsPeople.json), and no
// assembly declares that name, so BaseSpawner runs the entry with EntryFlags.InvalidType (BaseSpawner.cs:1130) and the
// spot stays empty. The type the data means is stock ElderVicaie (Engines/ML Quests/Definitions/Heartwood.cs:3387). Its
// DefaultName is "Elder Vicaie", title "the wise", a female elf with AI_Vendor; ServUO's Mobiles/NPCs/Vicaie.cs is the
// same NPC by name, title and sex (a BaseVendor that sells nothing and offers no quest). Pinned's data names
// eldervicaie nowhere, so under stock ModernUO Heartwood has no Elder Vicaie.
//
...
// "Server.Engines.MLQuests.Definitions.ElderVicaie", and nothing a player sees changes.
//
...
[TypeAlias("Server.Engines.MLQuests.Definitions.Vicaie")]
public partial class ElderVicaie
{
}
