...
// 0d1fffd9f) asks for "mallew" at Heartwood [7048,382] on Trammel and Felucca (shared/*/TownsPeople.json), and no
// assembly declares that name, so BaseSpawner runs the entry with EntryFlags.InvalidType (BaseSpawner.cs:1130) and the
// spot stays empty. The type the data means is stock ElderMallew (Engines/ML Quests/Definitions/Heartwood.cs:3321). Its
// DefaultName is "Elder Mallew", title "the wise", a male elf with AI_Vendor; ServUO's Mobiles/NPCs/Mallew.cs is the
// same NPC by name, title and sex (a BaseVendor that sells nothing and offers no quest). Pinned's data names
// eldermallew nowhere, so under stock ModernUO Heartwood has no Elder Mallew.
//
...
// "Server.Engines.MLQuests.Definitions.ElderMallew", and nothing a player sees changes.
//
...
[TypeAlias("Server.Engines.MLQuests.Definitions.Mallew")]
public partial class ElderMallew
{
}
