...
// 0d1fffd9f) asks for "aneen" at Heartwood [7053,338] on Trammel and Felucca (shared/*/TownsPeople.json), and no
// assembly declares that name, so BaseSpawner runs the entry with EntryFlags.InvalidType (BaseSpawner.cs:1130) and the
// spot stays empty. The type the data means is stock LorekeeperAneen (Engines/ML Quests/Definitions/Heartwood.cs:3584).
// Its DefaultName is "Lorekeeper Aneen", title "the keeper of tradition", a male elf with AI_Vendor; ServUO's
// Mobiles/NPCs/Aneen.cs is the same NPC by name, title and sex (a BaseVendor that sells nothing and offers no quest).
// Pinned's data names lorekeeperaneen nowhere, so under stock ModernUO Heartwood has no Lorekeeper Aneen.
//
...
// "Server.Engines.MLQuests.Definitions.LorekeeperAneen", and nothing a player sees changes.
//
...
[TypeAlias("Server.Engines.MLQuests.Definitions.Aneen")]
public partial class LorekeeperAneen
{
}
