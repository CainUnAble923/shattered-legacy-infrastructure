...
{
    /// <summary>
    ///     Clothing that can carry a Mondain's Legacy set bonus (S10).
    ///     <para>
    ///         ServUO puts this state directly on <see cref="BaseClothing" />. Here it lives on this
    ///         intermediate class for the reason given on <see cref="BaseSetArmor" />. Set content
    ///         derives from it; stock ModernUO clothing is untouched.
    ///     </para>
    ///     <para>
    ///         ServUO's clothing set state is the armour shape in full: per-piece and last-equipped
    ///         resistance arithmetic, <c>MixedSet</c>, <c>SetSelfRepair</c> and the absorption
    ///         attributes. The serialized layout is therefore identical to
    ///         <see cref="BaseSetArmor" />'s, fields 0-11.
    ///     </para>
    ///     <para>See <c>shard-migration/notes/s10-carriers.md</c> for the deviations.</para>
    /// </summary>
    [SerializationGenerator(0, false)]
    public abstract partial class BaseSetClothing : BaseClothing, ISetItem, IAbsorptionItem
    {
        ...
        ///     INERT: ServUO adds it to <c>ClothingAttributes.SelfRepair</c> inside
        ///     <c>BaseClothing.OnHit</c>, and ModernUO's <c>BaseClothing.OnHit</c> reads
        ///     <c>ClothingAttributes.SelfRepair</c> directly with no hook. Giving it effect needs a
        ///     three-line patch in the shape of <c>BaseArmor-set-self-repair.patch</c>, and S10 was
        ///     approved on zero patches, so the tooltip line is not emitted either (S1, D-2).
        ///     Deviation D-1 in <c>notes/s10-carriers.md</c>.
        /// </summary>
        [EncodedInt]
        [InvalidateProperties]
        ...
        ///     <see cref="BaseClothing" /> itself; here they ride on the carrier, as on
        ...
        public BaseSetClothing(int itemID, Layer layer, int hue = 0) : base(itemID, layer, hue)
        {
        ...
        public override void GetProperties(IPropertyList list)
        {
            ...
            // ServUO emits this block from inside BaseClothing.GetProperties, between the set
            // header and the race line. Same clilocs, a few lines lower. Cosmetic; D-4 in
            // notes/s1-armour-sets.md.
        ...
        /// <summary>
        ///     ServUO's <c>BaseClothing.GetSetProperties</c>, in its order: the resist block, then
        ///     the generic set lines. Note the order differs from <see cref="BaseSetArmor" />, which
        ///     follows ServUO's <c>BaseArmor</c> and emits the generic lines first. Both are as ServUO
        ///     has them.
        /// </summary>
        public virtual void GetSetProperties(IPropertyList list)
        {
            if (!_setEquipped)
            {
                // ServUO emits this here as well as inside SetHelper.GetSetProperties below, so a
                // Virtuoso clothing piece shows the cooldown line twice while the set is incomplete.
                // Reproduced as-is: it is what the source does.
            ...
            // ServUO adds "self repair ~1_val~" (1060450) here when SetSelfRepair != 0. Not emitted
            // while the value is inert; see _setSelfRepair.
